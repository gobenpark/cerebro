...
`)
	...
	for _, i := range tks {
		i.Date = i.Date.In(loc)
		fmt.Printf("%#v\n", i)
	}

	sort.Slice(tks, func(i, j int) bool {
		return tks[i].Date.After(tks[j].Date)
	})
	...
	for _, i := range tks {
		x = append(x, i.Date.In(loc).Format("2006-01-02 15:04:05"))
		y = append(y, opts.KlineData{Value: [5]int64{i.Open, i.High, i.Low, i.High, i.Volume}})
		v = append(v, opts.BarData{
			Name:  "volume",
			Value: i.Volume,
		})
	}
	...
	volumeBarChart.AddSeries("Volume", v,
